...
/**
 * True only for Gemini today — ElevenLabs' TTS here returns one MP3 buffer
 * with no incremental delivery, so there is nothing to stream. Checked
 * before calling synthesiseStream() so a caller can fall back to the
 * ordinary one-shot synthesise() instead of getting a confusing error.
 */
function supportsStreaming() {
  const provider = activeProvider();
  return Boolean(provider && typeof provider.synthesiseStream === 'function');
}

async function* synthesiseStream(text) {
  const provider = activeProvider();
  if (!provider) throw Object.assign(new Error('No speech provider configured'), { code: 'NOT_CONFIGURED' });
  if (typeof provider.synthesiseStream !== 'function') {
    throw Object.assign(new Error(`${provider.PROVIDER} does not support streaming synthesis`), { code: 'NOT_STREAMABLE' });
  }
  yield* provider.synthesiseStream(text);
}
...
module.exports = { config, isConfigured, cacheKey, synthesise, synthesiseStream, supportsStreaming, check, activeProvider, PROVIDERS };
